      ...
      <div className="pt-2 border-t border-gray-100 dark:border-slate-700/60 flex items-center justify-between mt-auto">
        ...
        <div>
          {isOutOfStock ? (
            ...
            <button
              onClick={onAdd}
              className="bg-purple-50 dark:bg-purple-950/60 hover:bg-purple-600 hover:text-white text-purple-700 dark:text-purple-300 font-black text-[11px] px-3 py-1.5 rounded-xl border border-purple-200 dark:border-purple-800 transition flex items-center gap-1 shadow-2xs cursor-pointer"
            >
              <Plus size={13} /> ADD
            </button>
          ) : (
            <div className="flex items-center gap-2 bg-purple-600 text-white rounded-xl px-2 py-1 shadow-sm">
              <button onClick={onRemove} className="hover:bg-purple-700 p-0.5 rounded transition cursor-pointer">
                <Minus size={12} />
              </button>
              <span className="font-black text-xs min-w-[14px] text-center">{quantity}</span>
              <button onClick={onAdd} className="hover:bg-purple-700 p-0.5 rounded transition cursor-pointer">
                <Plus size={12} />
              </button>
            </div>
          )}
